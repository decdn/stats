...
Cloudflare: the repo is two Workers — the page (`stats`, root `wrangler.jsonc`, built by `@opennextjs/cloudflare`; `open-next.config.ts` puts the ISR cache in the `decdn-stats-cache` R2 bucket, with an in-memory queue because `wrangler versions upload` — the preview-branch deploy — rejects Durable Object migrations) and the indexer (`decdn-stats-worker`, `worker/wrangler.jsonc`). The page deploys with the Workers Builds defaults (`pnpm run build`, `npx wrangler deploy`). `STATS_BASE_URL` is a dashboard var on the page Worker (`keep_vars` preserves it across deploys). See README "Deploying to Cloudflare".
