...
- **`worker/` — the indexer.** A separate pnpm workspace package (Cloudflare Worker, viem, wrangler) with its own `tsconfig.json`; the root `tsconfig`/eslint exclude it. On a cron it reads `stats.json` from an R2 binding, indexes `FeeRouter.Settled` from `lastBlock + 1` in capped chunks, and writes `stats.json` back (`worker/src/run.ts`). `worker/src/stats.ts` is pure (types + fold functions, no I/O) and is the shared schema — `lib/stats.ts` type-imports it. Bigints are decimal strings in base units. `stats.json` also records `feeRouter`/`startBlock`; if the worker's config names a different deployment it discards the file and re-indexes, so a contract redeploy only needs `FEE_ROUTER` and `START_BLOCK` (both in wrangler.jsonc `vars`, mirrored in `.env` for local runs) updated. `GET /stats.json` on the worker serves the bucket for local dev.
- **`lib/stats.ts` — `getStats()`.** Fetches `STATS_URL` with `next: { revalidate: 60 }` and returns a discriminated result: `ok` with the stats, `unconfigured` (env unset), or `unindexed` (worker 404, pre-first-tick). It throws on 5xx so ISR keeps serving the last good page. There is deliberately no mock fallback for settlements — the table renders labeled empty states instead. Formatting for display (`formatUsdc`, `truncateHex`, `formatUtcTime`) lives in `lib/utils.ts`.

The layering that matters:

- **`lib/mock.ts` — the static figures.** Metric values, deltas, chart series, and per-region delivery figures live here as typed exports (`Metric`, `MetricPoint`, `Region`, `RegionStats`) — the stand-ins for data that would be read from chain. Components import named exports directly; nothing takes props. Copy does **not** live here: headlines, labels, captions, micro-labels, and prose are hardcoded in the block that renders them, so changing what the page *says* means editing that block.
